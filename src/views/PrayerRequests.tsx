...
const SwipeableRequestItem = ({ req, onUpdateStatus, isAdminOrDeptHead, isLast }) => {
    const theme = useTheme();
    const controls = useAnimation();
    const [swiped, setSwiped] = useState(false);
    
    const handleDragEnd = async (event: any, info: PanInfo) => {
        if (!isAdminOrDeptHead || swiped) return;
        ...
        if (offset > threshold) {
            ...
            onUpdateStatus(req.id, 'praying');
        } else if (offset < -threshold) {
            setSwiped(true);
            await controls.start({ x: -500, transition: { duration: 0.2 } });
            onUpdateStatus(req.id, 'answered');
        } else {
        ...
        <Box sx={{ position: 'relative', overflow: 'hidden' }}>
            {isAdminOrDeptHead && (
                <>
                    ...
                    <Box sx={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '50%', bgcolor: 'info.main', display: 'flex', alignItems: 'center', px: 3 }}>
                        <Clock size={20} color="white" />
                        <Typography sx={{ color: 'white', ml: 1, fontWeight: 800, fontSize: '0.8rem', textTransform: 'uppercase' }}>Praying</Typography>
                    </Box>
                    {/* Right Background (Swipe Left) */}
                    <Box sx={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '50%', bgcolor: 'success.main', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', px: 3 }}>
                        <Typography sx={{ color: 'white', mr: 1, fontWeight: 800, fontSize: '0.8rem', textTransform: 'uppercase' }}>Answered</Typography>
            ...
            <motion.div
                drag={isAdminOrDeptHead ? "x" : false}
                dragConstraints={{ left: 0, right: 0 }}
                onDragEnd={handleDragEnd}
                animate={controls}
                style={{ backgroundColor: theme.palette.background.paper, position: 'relative', zIndex: 1 }}
            >
                <ListItem sx={{ py: 4, px: 3, alignItems: 'flex-start' }}>
                    ...
                    {isAdminOrDeptHead && (
                        <Stack direction="row" spacing={1} sx={{ display: { xs: 'none', sm: 'flex' } }}>
                            <IconButton size="small" color="info" onClick={() => onUpdateStatus(req.id, 'praying')} title="Mark as Praying"><Clock size={18}/></IconButton>
                            <IconButton size="small" color="success" onClick={() => onUpdateStatus(req.id, 'answered')} title="Mark as Answered"><CheckCircle2 size={18}/></IconButton>
                        </Stack>
                    )}
                </ListItem>
                {!isLast && <Divider />}
                    ...
                    <List disablePadding>
                        {filteredRequests.map((req, i) => (
                            <SwipeableRequestItem 
                                ...
                                isAdminOrDeptHead={isAdmin || isDeptHead}
                                isLast={i === filteredRequests.length - 1} 
                            />
                        ))}
