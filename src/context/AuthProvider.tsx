...
export const AuthProvider = ({ children }) => {
  ...
  const ensureProfileSync = useCallback(async (authUser) => {
    ...
    try {
      ...
      if (!profile) {
        if (process.env.NODE_ENV !== 'production') console.log("🛠️ Syncing profile for user...");
  ...
  const signup = async (email, password, name, department = null) => {
    ...
    setError(null);
    try {
      if (process.env.NODE_ENV !== 'production') console.log("Initiating signup for:", email);
      ...
      if (data.user) {
        ...
        if (data.session) {
          if (process.env.NODE_ENV !== 'production') console.log("Session detected, performing manual profile sync...");
          await supabase
            .from('profiles')
            .upsert({
              id: data.user.id,
