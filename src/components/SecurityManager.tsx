  ...
  if (isLocked) {
    return (
      <Box sx={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'background.default', p: 3 }}>
        <Paper elevation={0} sx={{ p: 8, borderRadius: '24px', border: '1px solid', borderColor: 'error.main', textAlign: 'center', maxWidth: 500 }}>
  ...
  return (
    <>
      ...
      <Snackbar open={alertState.open} autoHideDuration={6000} onClose={() => setAlertState({ ...alertState, open: false })} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
        <Alert severity={alertState.severity} variant="filled" sx={{ borderRadius: '12px', fontWeight: 700 }}>
          {alertState.message}
        </Alert>
      </Snackbar>
    </>
