...
const StudyDetailsDialog = ({ open, onClose, study, onUpdate }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" slotProps={{ paper: { sx: { borderRadius: '24px', p: 6 } } }}>
      ...
      {isEditing ? (
        ...
        <>
          ...
          <Grid container spacing={4} sx={{ mb: 8 }}>
              <Grid size={{ xs: 6 }}>
                  <Paper elevation={0} sx={{ p: 4, borderRadius: '16px', border: `1px solid ${theme.palette.divider}`, textAlign: 'center' }}>
          ...
          <Box sx={{ mb: 8 }}>
              <Stack direction="row" sx={{ justifyContent: "space-between", mb: 1 }}>
                  ...
                  <Chip label={study.progress === 100 ? "COMPLETED" : "IN PROGRESS"} size="small" sx={{ borderRadius: '12px', fontWeight: 900, fontSize: '0.6rem' }} />
              </Stack>
              <LinearProgress variant="determinate" value={study.progress} sx={{ height: 4, borderRadius: '4px' }} />
          </Box>
